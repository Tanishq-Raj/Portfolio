...
const preloadFonts = () => {
  try {
    ...
    // Only preload fonts that will be used immediately
    fontPreloadLinks.forEach(({ href, type }) => {
      try {
        // Check if the font is already in use in the document
        const fontAlreadyLoaded = document.fonts && Array.from(document.fonts).some(font => 
          font.family.includes('Gilroy') && href.includes(font.family.split('-')[1].toLowerCase())
        );
